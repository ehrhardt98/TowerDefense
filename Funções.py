...
def movimentar(invasores, displaySurface):
	...
	for i in lista_torres:
		displaySurface.blit(i.imagem, (i.posiçãox, i.posiçãoy))
	displaySurface.blit(invasores.sprite1,(invasores.posiçãox, invasores.posiçãoy))
	invasores.posiçãox = invasores.posiçãox + invasores.vx
	time.sleep(0.1)
	...
	for i in lista_torres:
		displaySurface.blit(i.imagem, (i.posiçãox, i.posiçãoy))
	displaySurface.blit(invasores.sprite2,(invasores.posiçãox, invasores.posiçãoy))
	invasores.posiçãox = invasores.posiçãox + invasores.vx
	time.sleep(0.1)
	pygame.display.update()
	...
	for i in lista_torres:
		displaySurface.blit(i.imagem, (i.posiçãox, i.posiçãoy))
	displaySurface.blit(invasores.sprite3,(invasores.posiçãox, invasores.posiçãoy))
	invasores.posiçãox = invasores.posiçãox + invasores.vx
	time.sleep(0.1)
	pygame.display.update()
	...
	while True:
		...
		if t == 1:
			...
			if y == 1:
				...
				t = 2			
		if t == 2:
			x = button(surface, "Começa", 360, 310, 50, 20, green, bright_green, "wave", 10, 0)
			if x == 1:
				t = 3
		if t == 3:
